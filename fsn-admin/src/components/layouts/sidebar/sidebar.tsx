...
export default function Sidebar({ children }: Props) {
  ...
  return (
    <div className="w-full flex flex-row mx-auto min-h-[calc(100vh-116px)]">
      {/* Sidebar on the left! */}
      <aside
        className="flex flex-col shrink-0 lg:w-[300px] w-[250px] sm:hidden xs:hidden md:block transition-all duration-300 m-0 fixed z-40 inset-y-0 left-0 bg-white border-r border-r-dashed border-r-white sidenav fixed-start overflow-y-scroll"
        id="sidenav-main"
      >
      ...
      <div className="flex w-full">
        ...
        <div className="m-4 grow w-[71.5%]">{children}</div>
      </div>
    </div>
  );
}
